...
const usage = () => {
  const options = [
    ...
    [
      "resourceGroup",
      "Name of your resource group (Defaults to name-environment)",
    ],
...
const qnaTemplatePath =
  argv.qnaTemplate ||
  path.join(__dirname, "DeploymentTemplates", "qna-template.json");
 ...
 */
const validateQnADeployment = async (
  client,
  resourceGroupName,
  deployName,
  templateParam,
 ...
 * @param {*} templateParam
 */
const createQnADeployment = async (
  client,
  resourceGroupName,
  deployName,
  templateParam,
...
const create = async (
  ...
  if (createQnAResource) {
    ...
    const qnaDeploymentOutput = await client.deployments.get(
      resourceGroupName,
      qnaDeployName,
    );
    if (
      qnaDeploymentOutput &&
      qnaDeploymentOutput.properties &&
      qnaDeploymentOutput.properties.outputs
    ) {
  ...
  if (createAppInsights) {
    ...
    if (appinsightsId && appinsightsInstrumentationKey && appinsightsApiKey) {
      ...
      if (botCreated.properties) {
        botCreated.properties.developerAppInsightKey =
