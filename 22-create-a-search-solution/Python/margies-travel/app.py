...
def search_query(search_text, filter_by=None, sort_order=None):
    try:
        ...
        results = search_client.search(
            ...
            select="url,metadata_storage_name,metadata_author,metadata_storage_size,"
            + "metadata_storage_last_modified,language,sentiment,merged_content,keyphrases,"
            + "locations,imageTags,imageCaption",
        )
        return results
...
def search():
    try:
        ...
        if "facet" in request.args:
            filter_expression = "metadata_author eq '{}'".format(request.args["facet"])
